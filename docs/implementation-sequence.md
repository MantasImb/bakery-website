...
- Decide where behavior-focused domain tests should live: colocate them with the module that owns the behavior, reserve root-level `__tests__` for app-level smoke, route, rendered layout, and cross-module integration tests, and keep direct tests of exported module-owned config beside the owning module.
